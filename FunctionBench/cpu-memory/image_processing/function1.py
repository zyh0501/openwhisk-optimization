...
def image_processing(file_name, image_path):
    ...
    with Image.open(image_path) as image:
        ...
        phase_latencies["filter"] = time() - t3

        t4 = time()
        path_list += ops.gray_scale(image, file_name)
        phase_latencies["gray_scale"] = time() - t4

        t5 = time()
        path_list += ops.resize(image, file_name)
        phase_latencies["resize"] = time() - t5

    latency = time() - start
    phase_latencies["function_execution"] = latency
...
def _run_core(event):
    ...
    (
        ...
        path_list,
        image_processing_phase_latencies,
    ) = image_processing(object_key, download_path)
    latencies["function_execution"] = image_processing_latency
    latencies["image_processing_phases"] = image_processing_phase_latencies
...
def _profile_with_scalene(event):
    outfile = os.path.join(tempfile.gettempdir(), "scalene-image-processing.json")
...
def image_processing_profiled(file_name, image_path):
    phase_latencies = {{
        "open_image": 0.0,
        "flip_left_right": 0.0,
        "flip_top_bottom": 0.0,
...
def run_core_profiled(ev):
    ...
    image_processing_latency, path_list, phase_lat = image_processing_profiled(
        object_key, download_path
    )
    latencies["function_execution"] = image_processing_latency
    latencies["image_processing_phases"] = phase_lat
